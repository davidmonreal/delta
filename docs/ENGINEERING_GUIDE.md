...
- Smoke test flow (manual, with temporary superadmin):
  - `npm run user:create -- --email smoke+<ts>@example.com --password <pass> --name "Smoke Superadmin" --role SUPERADMIN`
  - Login and visit `/`, `/admin/users`, `/admin/upload`, `/client/<clientId>`
  - Delete the smoke user afterwards.
